...
//public interface fn<FuncType extends fn<FuncType>> extends UnaryOperator<FuncType>{
	...
	/** FIXME should long lid() be moved to Ocns aka occamsfuncer local namespace?
	<br><br>
	Local ID, readable by op.nondet. For example, you might store this in a file of blocks of 3 longs,
	one of them being the id of the node and the other 2 being its 2 childs, and the file is binary searchable
	and garbcolable by copying it to another file while keeping only those reachable from a certain node.
	You might also want refcount in it but that requires RandomAccessFile instead of just appending to file and read random access.
	<br><br>
	TODO can hold literals of 1 2 4 8 16 and 32 bits, so to store 256 bits for example
	you would use 8 longs that are 32 bit literals and 4 nodes above that (3 longs each)
	and 2 nodes above that and 1 node above that, so total 7 nodes that are 3 longs each so 21 longs (1344 bits) to store 256 bits.
	Can reach 99.99% storage efficiency in other storage systems using optimizations planned for cbt, but this is very basic experiments for now.
	WAIT... thats ignoring the (pair myLeft) in (pair myLeft myRight), but pairOfPairDepth optimization can solve that
	for the cost of 1 (or a few?) of the 64 bits in long. But for now just pay the higher cost (maybe around 2000 bits to store 256 bits?).
	Actually for now dont even store literals and directly store the whole forest down to leaf.
	<br><br>
	Also might need 4 childs for callquad (callpair is a callquad whose stack and cacheKey is null) and a long for header
	and an int or long for refcount.
	<br><br>
	BUT... cant dedup just by binary searching such a file. Need hash buckets for that.
	Use 4 longs: [hashBucket id leftChildId rightChildId], where hashBucket is deterministicly derived from leftChildId and rightChildId.
	<br><br>
	Doesnt affect global id. This is to avoid triggering lazyHash which is expensive
	and uses 512 bit ids (448 of which is occamsblob or up to 256 of which is literal).
	Its recommended to use utcnano (number of nanoseconds since Y1970) as these ids
	and to remember the last utcnano returned so can use max(prevId+1,utcnano()) as next id.
	That gives you a billion new potential ids per second and they all know their creation time.
	*/
	public long lid();
	...
	*/
	...
	public default fn e(Object param){
		return e(w(param));
	}
	
	/** wrap, not calling it. This might be better as a static func, but this is an interface and I cant implement it here without knowing other details
	of a specific implementation such as occamsfuncer.impl vs occamsfuncer.simpleSlowPrototype might optimize the wrapping differently.
	For example, if wrapMe instanceof int then its a cbt32 (such as a occamsfuncer.impl.fn.SmallBlob which has a long for data and a byte for height).
	*/
	...
	/** get the universal func, which always takes 15 params and is a universal lambda function and a pattern calculus function. */
	public fn u();
	
	/** get the identityFunc.
	OPTIMIZE: Implementing classes should override this for efficiency cuz otherwise it will be created from combos of leaf(),
	which will get the same fn (by ==) as whatever you're using for identityFunc but will happen slower.
	*/
	public default fn identityFunc(){
		fn u = u();
		return u.e(u).e(u.e(u)).e(u).e(u).e(u.e(u)).e(u).e(u).e(u).e(u).e(u).e(u).e(u).e(u).e(u);
	}
	
	/** same as func() except is never null. leaf.func is identityFunc, and leaf.param is leaf. */
	public default fn Func(){
		return isLeaf() ? identityFunc() : func();
	}
	
	/** same as param() except is never null. leaf.func is identityFunc, and leaf.param is leaf. */
	public default fn Param(){
		return isLeaf() ? this : param();
	}
	...
	*/
	...
	/** same as byteAt(arrayIndex<<3) like byte[arrayIndex]. May be more efficient if unwrap() instanceof byte[] */
	public default byte ByteAt(int arrayIndex){ return byteAt(arrayIndex<<3); } 
	
	/** same as shortAt(arrayIndex<<4) like short[arrayIndex]. May be more efficient if unwrap() instanceof short[] or char[] */
	public default short ShortAt(int arrayIndex){ return shortAt(arrayIndex<<4); }
	
	/** same as charAt(arrayIndex<<4) like char[arrayIndex]. May be more efficient if unwrap() instanceof char[] or short[] */
	public default char CharAt(int arrayIndex){ return charAt(arrayIndex<<4); }
	
	/** same as intAt(arrayIndex<<5) like int[arrayIndex]. May be more efficient if unwrap() instanceof int[] */
	public default int IntAt(int arrayIndex){ return intAt(arrayIndex<<5); }
	
	/** same as longAt(arrayIndex<<6) like long[arrayIndex]. May be more efficient if unwrap() instanceof long[] */
	public default long LongAt(int arrayIndex){ return longAt(arrayIndex<<6); }
	
	/** same as floatAt(arrayIndex<<5) like float[arrayIndex]. May be more efficient if unwrap() instanceof float[],
	especially cuz of raw vs normed form of float as int bits and the requirement of determinism.
	*/
	public default float FloatAt(int arrayIndex){ return floatAt(arrayIndex<<5); }
	
	/** same as doubleAt(arrayIndex<<6) like double[arrayIndex]. May be more efficient if unwrap() instanceof double[],
	especially cuz of raw vs normed form of double as long bits and the requirement of determinism.*/
	public default double DoubleAt(int arrayIndex){ return doubleAt(arrayIndex<<6); }

...
public class OcfnUtil{
	...
	*/
	...
	/** leaf and u both mean the universal function (the 15 param function that is occamsfuncer,
	which is a universal lambda function and a pattern calculus function).
	*/
	public static final fn leaf = u;
	...
	*/
	...
	//TODO remove this
	//just for testing how fast it would be to call sha3_256 every step instead of just when trigger lazyhash
	static final MessageDigest md;
	//the first 4 bytes of this are modified in the test
	static final byte[] mdIn = Text.strToBytes("0000abcdefgajksdjfalk4ejr5lkwsjedfkwl43rsldkfjasdlkfjasdfsd");
	static long mdSum;
	static{
		try {
	...
	*/
	public static fn step(fn evalingState, boolean isDeterministic){
		...
		/*int ih = System.identityHashCode(evalingState);
		mdIn[0] = (byte)(ih>>>24);
		mdIn[1] = (byte)(ih>>>16);
		mdIn[2] = (byte)(ih>>>8);
		mdIn[3] = (byte)ih;
		byte[] hash = md.digest();
		mdSum += hash[0]; //to make sure it doesnt optimize it away by not doing it since it has no effect. it has this effect.
		
		/*
		Without the md.digest code:
		steps=92313
		seconds=0.06763580000000001
		stepsPerSec=1364854.1157197813
		objectsInCachePerSec=238335.31946099547
		
		With the md.digest code:
		steps=92313
		seconds=0.1528552
		stepsPerSec=603924.4984796068
		objectsInCachePerSec=105459.28434230566
		
		Thats 44% as fast. I can maybe live wit it since the number crunching will be nearly the same speed,
			but it migbht be a problem with acyclicflow and raw float bits etc. 
		
		*/
		//todo consider the above comment
		
		
		countSteps_todoRemoveThisVar++;
		if(lgStep) lg("start step: "+evalingState);
		if(evalingState.isDone()){
			if(lgNoOp) lg("Why are you calling step on an isDone? "+evalingState);
			return evalingState;
		}
		fn ret = null;
		fn allCurriesExceptLast = L(evalingState), lastParam = R(evalingState);
		if(evalingState.isHaltedAbove()){
			...
			//In practical implementations, a node will know most of this stuff cached recursively from its L,
			//such as which of 16 ops it is, number of curries left before eval, and a pointer to its funcBody if its a bigCall<1..7>,
			//so it wont have to read all 15 params here.
			fn list = evalingState;
			fn p = R(list);
			list = L(list);
	...
	*/
	...
	/** TODO create a recur (no number) that checks which of op1 to op7 it is which is the number of params it takes */
	private static fn recur3;
	public static fn recur3(){
		if(recur3 == null) recur3 = progn(L,R,L,L); //in (pair allCurriesExceptLast lastParam) get (L (L allCurriesExceptLast))
		return recur3;
	}
	...
	private static fn linPlusOneTwoscomplement;
	...
	*/
	public static fn linPlusOneTwoscomplement(){
		if(linPlusOneTwoscomplement == null) linPlusOneTwoscomplement = lambda(1,IF(
			ST(A,p(15)), //if param is leaf/u aka number with no digits, which normally happens just after the highest digit
		...
		return linPlusOneTwoscomplement;
		
		
		/*recur
			
		FIXME does recur2 actually mean it takes 3 params, given that a context param (can be u)
		is normally the first param in every lambda1 to lambda7 (the 7 build-your-own-lambda opcodes)?
		*/
	}
	...
	*/
	public static fn linPlusTwoscomplement(){
		...
		//if(linPlusTwoscomplement == null) linPlusTwoscomplement = TODO;
		//return linPlusTwoscomplement;
		
		//TODO use churchEncoding (have testcases for it in some fork of occamsfuncer) to create lin numbers of n Fs (0s)
		//then use those in linPlusTwoscomplement and other lin funcs, to pad things, maybe.
		*/
	}
	
	/* from other fork of occamsfuncer:
	...
	*
	public static final Node linPlusOne = e(
		...
		IF(
			...
			thenConst(f(T,nil)),
			then(IF(
				ST(L,p9), //get lowest digit as T or F.
				...
				thenT( //lowest digit is F. Replace it with T.
					T,
					ST(R,p9) //leave rest of the linkedlist (higher digits) as it is
				)
			)
		)),
	...
	*/
	...
	/** Doesnt work yet, but calling the java func step does.
	The lazyCall and step ops are the only 2 metaOps that take more than 1 param (2 params each). Might become multiple step calls depending on DedupLevel.

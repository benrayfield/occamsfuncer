...
public class Nondet{
	...
	public static fn nondet(fn type, fn instance, fn param){
		...
		if(Gas.forceDeterminism){
			...
			if(type == leaf){ //not using type
				String instanceS = str(instance);
				if(instanceS.equals("solve")){
					throw new Error("FIXME see comments below. What other params should solve take?");
					
					//return GoalSolver.solve(param);
					
					/*FIXME change the solve API to also take a param (what datastruct?)
					of example things to try calling on eachother
					that are likely to be useful in a solution,
					which caller might know from the relevant things
					caller is trying to solve about
					or from combos of earlier (not as high scoring) solutions.
					
					The 5 actions on the "fn tape" can loop thru all possible functions,
					...
					The fn tape register will start containing a typelist of fns to try combos of.
					It will start by estimating how much Gas to use on each possibility,
					or maybe it will split the Gas 5 ways for the 5 possible actions,
					recursively, though that seems unfair to those scoring higher
					if its organized into calling solve on subtasks and knows
					what the pieces mean.
					It also seems very very high branching factor
					to do almost anything beyond the most trivial things,
					but as it gets smarter it might overcome that.
					
					Maybe there should be some kind of neuralnets andOr statistics
					organizing the amount of Gas risked in the "game tree" (actually a forest)
					of possible solutions to the goal
					(starting from the "fn tape" register being a typelist of
					fns expected to maybe be part of the solution or used in building it)
					... and those neuralnets or statistics would be about
					the results of measureFuncs (any func that returns a double,
					so a goalFunc is a measureFunc) so every possible func
					can be viewed as an infinite dimensional vector of
					the return values of measureFuncs about it,
					and thats where neuralnets or statistics could hook in
					to calling func on func to create/find func recursively.
					*/
					
				}else if(instanceS.equals("getVar")){
					String paramS = str(param);
					if(paramS.equals("solver")){
						return GoalSolver.solver();
					}
				}
			}else{
				String typeS = str(type);
				if(typeS.equals("setVar")){
					throw new Error("This must only work by digitalSignature (default is ed25519 but integrate with Network.java mutableWrapperLambda *MWL funcs maybe) in one of the (use cccc() or however many params around the (nondet...)), if its allowed at all, since not just any func should be able to change the behviors of nondet as other funcs could call it. global vars go against the design of occamsfuncer and cause caching problems if set more than once before that part of cache is cleared, but since its in nondet, anything goes. TODO implement that?");
				}/*else if(typeS.equals("getVar")){
					String instanceS = str(instance);
					if(instanceS.equals("solver")){
						return GoalSolver.solver();
					}
				}*/else if(typeS.equals("ocfnplug")){
					...
					*/
					...
					String instanceS = str(instance);
					//Its recommended that salt be the first param after funcBody in curry calls
					//that can possibly call wallet or spend,
					//and that some system be designed to automatically forkEdit salt
					//by a different strongPseudorandom unitary transform depending on
					//which path through code leads to it such as is it the first or second
					//child call of a curry that calls multiple curries.
					if(instanceS.equals("wallet")){
						//ignore salt here but Cache.java does not ignore it, as in 
						//(nondet ;limitComputeResources ;wallet salt).
						return w(Gas.top);
				...
				}/*else if(typeS.equals("solve")){
					fn goal = instance;
					//fn salt = param;
					//fn salt = leaf;
					//return GoalSolver.solve(goal,salt);
					return GoalSolver.solve(goal);
				}*/
			}

...
which means they are made only of calls of universal lambda function
(including arrays which represent
certain forest shapes of that universal lambda function),
but the point is not to include any calls of nondet
except for Wallet and Spend calls to limit compute resources.
The combos of universal lambda function (trinary forest shapes)
will often be automatically optimized or manually optimized in
occamsfuncerVM code,
or if manually optimized then could put these "temp" funcs in
Compiled objects of the user level code
so in any occamsfuncerVM that doesnt have those Compiled objects
it will still work and may even have different Compiled objects
optimizing it a different way and will all be compatible
nomatter which Compiled who is using.
Its ok to fork an occamsfuncerVM to
...
public class TempOcfnplugs{
	...
	public static fn ocfnplugDoubleMultiply(fn madeByCurry){
		fn x = getp(4,madeByCurry);
		fn y = getp(5,madeByCurry);
		return w(x.R().doubleAt(0)*y.R().doubleAt(0));
	}
	...
	*/
	...
	/** save all as 1 bitstring. You can write the returned bitstring
	to a file, load it later
	In more advanced kinds of saving/loading later they will be stored sparsely
	on Internet, haddrives, and memory of many computers in realtime.
	*/
	public static fn ocfnplug_saveDense(fn madeByCurry){
		fn meAndEverythingReachable = getp(4,madeByCurry);
		throw new Error("TODO use varint ptrs to earlier in file?");
	}
	
	/** inverse of ocfnplug_saveDense.
	If load multiple partially overlapping bitstrings,
	they will dedup some of their childs, but only dedup */
	public static fn ocfnplug_loadDense(fn madeByCurry){
		fn bitstring = getp(4,madeByCurry);
		throw new Error("TODO");
	}

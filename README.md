...
TODO rewrite this big paragraph, some parts are more speculative or not relevant to this part.
Privacy policy and legal things you might have to deal with: The system does only 1 thing: when given 2 binary forest nodes, does 1 small compute step toward giving you back a binary forest node, and waits for further instruction. Specificly, it does not have any system permissions (such as execute), but external systems may choose to react to whats observed inside it at their own responsibility of acting only based on strong math proofs, and stay inside a security layer similar to how webpages cant escape your browser except its like 1 big browser running on everyone's screen at once, and specificly this security layer (which is a math spec and does not include any possibly buggy implementations, which can instead be turing-complete-challenge-responsed to converge toward global sync and the spec) is designed to safely contain any possible virus, malicious AI, botnet, ransomware, etc (if those happen to be built, not that I'm suggesting it) while simultaneously safely operating dangerous equipment (such as a dyson ring or antimatter bottle (antimatter being possibly synthesizable by neutrino sonoluminescence by use of magnetic resonance machines tuned to be magneticly/weakNuclearForce neutral but nonneutral for resonance strong-nuclear-force, or something like that) or graygoo operating system, as the spec is so simple, one constant-time-calculation happening many many times in parallel and sequentially, it could potentially be implemented for example between 2 vibrating surfaces which squash various atoms together to detect and adjust for errors while printing diamond thread faster than a bullet, for example, such a printer and its computing parts being one integrated piece) by precise math specs, or whatever people may want to use it for. The prototype code is opensource MIT licensed, but lambda functions themselves are a part of math, and nobody owns math. There is no central authority, and the plan is for many forks of the occamsfuncer opensource to work together, further strengthening that. The peer-to-peer network will be a public space where people can build and use things together using immutable objects made of other immutable objects. Every computer will have a private area and a border between that and the public space. The system does not, when you first download it, contain encryption but is a computer and every computer is capable of being used to build encryption, so if you need privacy across a public space, thats how you (or many people building and sharing functions together) would do it and be sure that the builders of the system were not able to put backdoors in it (but users may choose to create things with or without backdoors, as a computer can do all possible patterns of information) since those builders only make the lambdas work as the math says exactly what they must do. The user(s) of that computer (no back doors known to the builders of the system, unless your computer was already backdoored by some other program) can choose to manually move things from their private space into the public space, but nobody can force anyone to remove something from public nomatter how it got there, nomatter the reason, cuz that may break things that are built with other things deeply which happens many times per second. Nobody can force something to stay in the public space as it normally fades away (is uncached) whenever everyone stops using all things which depend on it, but there is no central way to detect or cause that. Thats just how lambda functions work. Its a lazyEvaled merkle forest. Users may also authorize any chosen function to act on their behalf to choose what to put from private to public, such as a function that only identifies pictures of cats or a function which copies your game controller movements during a live playing of a game with other people, or new games themselves and millions of forks of the games into variants of the games. DMCA Takedown demands are not usually applied to the internal workings of computers since if you change a few randomly selected bits in memory a program usually crashes, but in this system such internal workings and Human understandable content are all lambdas, so if someone makes such a demand, that would be between them and each individual in the peer to peer network they are claiming it against since there is no central authority, and those who want to try to comply may together try to create a function which does whatever is being demanded (or even better, those who are demanding it can be precise in what they want by creating the function themselves and demonstrating to others how it can be done), and anyone may choose to use or not use that function in whatever ways they want, but probably they will find that those are unreasonable demands on a technical system which creates new functions millions of times per second per computer, sometimes billions or trillions if you count GPU, and scanning each of those for a whole library of existing content would make the system run impractically slow, and it would damage the system wherever things have become built which include many parts, if you for example say that a certain number is your stolen credit card number and therefore anyone who adds 2 other numbers together better make sure they dont sum to that. Hopefully the system will evolve into ways that millions or billions of people can work together to solve whatever problems come up in the system.

occamsfuncerV3 test cases: https://github.com/benrayfield/occamsfuncer/blob/master/immutable/occamsfuncer/v3/spec/sparseTuringMachine/test/TestOcfn3FormalVerifySpec.java

...
-- everyone age 5 and up, for the simpler uses, as drag-and-droppable functions become part of the internet as easily as memes spread, copy/pasteable and you drag a function onto a function to create or find a function, which may do anything such as draw a mustache on whatever picture its dragged to, or may be a game or a bot or a sentence or a popup menu or a musical instrument or could be anything. You cant break it, and it cant break anything else. You cant even crash the browser tab with it using an infinite loop. The space of all possible functions and turing completeness available everywhere by drag-and-drop, even if you dont know which drag-and-drops would do advanced things. You could, for example, drag-and-drop basically #long_hashtags_with_numbers345345346435634534345389475389475349857324583248327453485734857 (viewed in other ways which are themselves functions), each representing a lambda function, onto eachother across multiple social networks and create a working, scaleable, load balanced, industrial strength security (for example, since it is a pattern calculus function, it does not have a root so can not be rootkitted (even if run in a computer which is already rootkitted (which may be the case, given how many binary blobs and secrets are in the tech industry) it is firewalled (similar to "tilt bits" (except with exactly 0 false-positives (and more possible true-positives than there are atoms in the visible universe) which are themselves a form of security weakness) but potentially someday homomorphicly depending what it derives) from that rootkit by turing logic), but flawed implementations of the math may locally be, and even if they are they cant hide that from the rest of the network due to turing-complete-challenge-response except if such rootkits choose to perfectly implement the spec)... aka this thing in theory eats botnets, skynets, rootkits, malicious installations of wrongly trusted antivirus programs, etc... like the first multicelled life eats bacteria, within formally-verifiable possible behaviors and please do be very careful together toward the goal of that nobody has to be careful as things just work..., competing social network, or tool of any kind, and deploy the thing before the echo of your mouse click from drag-and-drop has gone quiet (cat walks on keyboard, or 5 year old does what the tv tells them to, and instead of malware being secretly installed into the computer, malware is selected in sandbox, scientificly peer reviewable, and can permanently store it or uncache it safely such as like a "head on a pike" in a video game hey everyone look at this malware that dared to show itself to me everyone come poke it with a stick or see what it does in other emulated combos), and a million people could do that 1000 times per second each and it wouldnt even slow the system down as its made of very very very tiny forkEditable pieces of logic. Or more likely, you might just create ever more interesting variations of mustaches on cat memes, including the ability to make a meme of a cat grabbing a mouse and doing such drag and drop to forkEdit another cat meme or other small simple fun things. Self-aware living memes. You see those ( ) S P #equals etc, that looks like random nonsense in the pic... Thats like their DNA, except random evolution is not necessary as it is precise enough to represent the infinite number of exact binary digits of pi as a function of integer (which digit do you want) to 0 or 1. To a 5 year old, it might look like cartoons and toys. To a scientist it might look like the inside of a borg cube. To a security researcher it might look like a bunch of formal-verification proofs that refer to eachother. Whether some individual person chooses to, go to annoying steps to execute something which they see "on a pike in a video game" as many others think they shouldnt (and similarly to opensource fork the whole system to have no such protections at all), is still their own choice, in the ways of opensource, but it should by design be that they can not accidentally or thru ignorance do so (I know I've rambled in this long text about occamsfuncer, but I do plan to simplify the explanation and do more by example sooner, and in general I am against the "agree" button in licenses and instead by a few words here and there as relevant that all minds involved understand whats relevant and choose based on that or back out), nor should everyone have such execute permission such as one may request to the network a limited "remote desktop" etc in which they may not opt into such forks (suggested use: at public kiosks at your business (even Richard Stallman wrote he is ok with limited permissions at a kiosk since its not his own computer), or for the computer illiterate), from any of opensource forks of the p2p network some of which may work together more or less, as it should be in theory a well balanced nash equilibrium among the space of all possible interactions between people computers etc. In theory. Also, sometimes crowds of people andOr bots act irrationally, so I would suggest not granting execute permission to anything in such networks ever for any reason, except within the (TODO) global sandbox and math specs that opensource forks may in theory evolve safely within. One of the advantages of this system over others is that execute permission is not needed for almost anything, and similarly has no ability to modify anything anywhere except to suggest a "what if" and "what if after that" and so on, even going deeply into many people and their computers exploring a high dimensional turing complete space of "what if", which anyone may fork or merge with others as they like... (TODO multiplayer), technically speaking, "what if you had virusX" is data compared to execute virusX, and if we just keep it at the "what if" level forever, we can call it a turing complete sandbox with practically all abilities as outside the sandbox as could be hooked in as those with such abilities and liabilities choose or do not choose to risk on that.
...
-- REWRITE: nondet solve.
-- REWRITE: sandbox across many computers together
--
--
--
--
...
https://github.com/benrayfield/occamsfuncer/blob/master/immutable/occamsfuncer/simpleSlowPrototype/TestSpec.java
...
or such as would certainly be found eventually (if we had infinite compute resources) by, quoted from much farther below, "Using a sequence of these 5 actions, anything can be created: fnTapeLeft fnTapeRight copyRegisterToCenterOfFnTape copyCenterOfFnTapeToRegister callFnInRegisterOnFnAtCenterOfTapeAndReplaceRegisterFnWithWhatThatReturns", is a way of using 2 linkedlists of functions, starting from completely empty except the infinite size turing-tape of functions and the 1 register is filled with occamsfuncer (a constant universal function), to choose 1 of 5 actions repeatedly to find any possible function in the space of all possible functions in no more than "some small constant" times number of steps later than the perfect compression like in https://en.wikipedia.org/wiki/AIXI (and if used in combination with dropping in literal bitstrings such as prefixed by their varint length, could in theory emulate every known kind of compression and specific compressed file in no more than smallConstant + otherSmallConstant * sizeOfActualCompressedFileOfAnyKind_includingSizeOfDecompressingSoftwareAndOSandCircuitLayoutAndStandardModelOfPhysicsEtc, BUT since I do not have unlimited computing power I tend to focus on things that can eventually be experienced by Humans or machines they may be able to use together someday) if one were to be "crazy rich" in computing cycles and memory instead of sacrificing precision and size for efficiency. For example, the best possible move in chess, is some certain move even though it would take exponential compute cycles to know. It would probably only take about 64*numberOfMoves * smallConstant amount of memory, but exponential compute cost. It may already be known by someone, but chess is still a very small board compared to the space of all possible functions.
